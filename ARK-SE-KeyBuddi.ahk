InstallDirectorySuffix:= "\KeyBuddi\ARK\"
UniqueShortcutString:= "KeyBuddi - Ark" ; This should match KeyBuddi_Subtype
#Include Lib\Initialize_Defaults.ahk
#Include Lib\defaultKeyBuddiAvatar.ahk

Sleep, 800
KeyBuddi_Subtype = KeyBuddi - ARK:SE 
KeyBuddi_Version = 0.08 ;Used for display in application
; KeyBuddiThisVersionNote=A new version of KeyBuddi (0.08) is available. Also found at
LogAppTitle = %KeyBuddi_Subtype%
LogAppVer = %KeyBuddi_Version%
...
version := "0.08", company := "Wrongtown"    ; Keep these lines together
;@Ahk2Exe-Let KeyBuddi_Version=%A_PriorLine~U)^(.+"){1}(.+)".*$~$2% ; Keep these lines together
;@Ahk2Exe-Let cy=%A_PriorLine~U)^(.+"){3}(.+)".*$~$2% ; Keep these lines together
;@Ahk2Exe-ExeName %A_ScriptDir%\KeyBuddi-Ark\KeyBuddi - ARK
;@Ahk2Exe-AddResource *24 %A_ScriptDir%\resources\ARK.ahk.manifest, 1
;@Ahk2Exe-Let var = KeyBuddi - ARK, var1 = Copyright`, Troy Hall
...
SplashImage, pr-splash.jpg, b fs18, %KeyBuddi_Subtype% v%KeyBuddi_Version% `n by Wrongtown 2021 `n `n To stop the script at any time`npress Ctrl+F10`n
...
Gui, font,, Helvetica ; Preferred font (size, weight & quality are all inherited).
; Gui, Add, Text, xs
Gui, Add, Text, xs r1 , `nAbout KeyBuddi
...
WinSet, Transparent , 235, About KeyBuddi, , , 
WinSet, Enable ,, About KeyBuddi, , , 
